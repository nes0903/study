...
flowchart TD
    A["클래스(Class)"]
    A --> N1["공통된 속성과 연산을 갖는 객체의 집합이다."]
    A --> N2["클래스에 속한 각각의 객체를 인스턴스라고 한다."]
    A --> N3["객체지향 프로그램에서 데이터를 추상화하는 단위이다."]
```

- 클래스는 공통 속성과 연산을 갖는 객체들의 추상적 설계도이며, 클래스에 속한 실제 객체를 인스턴스라고 한다.
- 이 챕터는 `객체지향` 영역에 속한다.
...
- 클래스는 객체를 만들기 위한 틀 또는 타입이다.
- 속성은 객체가 가지는 데이터이고 연산은 객체가 수행할 수 있는 행위다.
- 인스턴스는 클래스라는 틀로부터 생성된 실제 객체다.
...
- 클래스 = 붕어빵 틀, 객체/인스턴스 = 만들어진 붕어빵으로 비유하면 쉽다.
