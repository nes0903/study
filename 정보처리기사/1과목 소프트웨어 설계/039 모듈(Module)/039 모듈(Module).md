...
flowchart TD
    A["모듈(Module)"]
    A --> N1["모듈화를 통해 분리된 시스템의 각 기능들이다."]
    A --> N2["단독으로 컴파일이 가능하다."]
    A --> N3["재사용 할 수 있다."]
    A --> N4["다른 모듈에서의 접근이 가능하다."]
```

- 모듈은 모듈화를 통해 분리된 시스템 기능 단위로, 독립적으로 컴파일 가능하고 재사용 가능하며 다른 모듈에서 접근 가능한 구성요소다.
- 이 챕터는 `모듈` 영역에 속한다.
...
- 모듈은 프로그램을 구성하는 독립적 기능 단위다.
- 모듈은 명확한 입력과 출력을 가져야 한다.
- 좋은 모듈은 내부 응집도가 높고 외부 결합도가 낮다.
- 모듈은 컴파일, 시험, 재사용, 유지보수의 단위가 될 수 있다.
...
|---|---|
| `모듈` | 분리된 기능 단위 |
| `컴포넌트` | 배포/조립 가능한 더 큰 단위로 쓰이는 경우 많음 |
| `함수` | 모듈 내부의 세부 처리 단위일 수 있음 |
