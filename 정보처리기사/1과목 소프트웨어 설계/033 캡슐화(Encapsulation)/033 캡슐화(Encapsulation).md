...
flowchart TD
    A["캡슐화(Encapsulation)"]
    A --> N1["데이터와 데이터를 처리하는 함수를 하나로 묶는 것이다."]
    A --> N2["외부 모듈의 변경으로 인한 파급 효과가 적다."]
    A --> N3["인터페이스가 단순화된다."]
    A --> N4["재사용이 용이하다."]
```

- 캡슐화는 데이터와 그 데이터를 처리하는 함수를 하나로 묶어 외부 영향과 인터페이스 복잡도를 줄이고 재사용성을 높이는 객체지향 원리다.
- 이 챕터는 `객체지향` 영역에 속한다.
...
- 상태와 행위를 한 객체 안에 모은다.
- 외부는 객체가 공개한 메서드를 통해서만 상태를 다루게 한다.
- 내부 구현이 바뀌어도 외부 인터페이스가 유지되면 영향이 줄어든다.
