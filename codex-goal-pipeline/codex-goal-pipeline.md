...
- `Feature::Goals`: `/goal` 및 goal tool 노출을 켜는 experimental feature flag다.
- `thread/goal/set|get|clear`: TUI나 외부 클라이언트가 app-server에 목표 상태를 조작할 때 쓰는 JSON-RPC method다.
- `get_goal/create_goal/update_goal`: 모델에게 노출되는 tool이다. `update_goal`은 완료 처리만 허용하고 pause/resume/budget_limited는 사용자 또는 시스템이 제어한다.
...
- 목표 기능은 `goals` feature flag가 꺼져 있으면 동작하지 않는다. 이 flag는 experimental stage이며 메뉴 설명도 "persistent goal"과 "continue over time"에 맞춰져 있다.
...
- 모델 tool은 보수적으로 설계되어 있다. `create_goal`은 명시적 요청이 있을 때만 새 goal을 만들고, `update_goal`은 complete만 허용한다.
...
mindmap
  root((Codex /goal))
    ...
    ModelTools
      get_goal
      create_goal
      update_goal complete only
```

## 8. 다른 에이전트에서 직접 구현하기 위한 세팅

- 다른 에이전트에서 이 파이프라인을 직접 구현하거나 재현하려면 먼저 목표 기능을 켜야 한다. `goals`는 기본 비활성 experimental feature라서 `~/.codex/config.toml`에 아래 설정을 넣거나, 실행 시 `--enable goals` 또는 `-c features.goals=true`를 넘겨야 한다.
...
- app-server API로 구현하는 에이전트라면 initialize 시 `capabilities.experimentalApi = true`가 필요하다. `thread/goal/set|get|clear`가 protocol에서 experimental method로 표시되어 있기 때문에 opt-in 없이 호출하면 app-server가 거절한다.
...
```mermaid
flowchart TD
    A["구현 에이전트 준비"] --> B["Codex 실행 옵션 또는 config.toml"]
    B --> C["features.goals = true"]
    C --> D{"접근 방식"}
    D -->|TUI 검증| E["/goal 명령 직접 입력"]
    D -->|API 구현| F["app-server initialize"]
    F --> G["capabilities.experimentalApi = true"]
    G --> H["thread/start: ephemeral false, cwd 지정"]
    H --> I["thread/goal/set|get|clear 호출"]
    I --> J["thread/goal/updated, cleared 처리"]
    J --> K["turn lifecycle와 continuation 검증"]
```

### 구현 체크리스트

- `goals` feature flag가 실제 실행 프로세스에 적용되는지 `/experimental`, `codex features list`, 또는 goal API 호출 결과로 확인한다.
...
{ "method": "initialize", "id": 1, "params": {
  "clientInfo": { "name": "goal-agent", "title": "Goal Agent", "version": "0.1.0" },
...
```json
{ "method": "thread/goal/set", "id": 3, "params": {
...
{ "method": "turn/start", "id": 4, "params": {
  "threadId": "응답으로 받은 thread.id",
  "input": [{ "type": "text", "text": "Start working toward the active goal." }]
} }
```
...
- [Core goal runtime](/Users/nes0903/Documents/reference/codex/codex-rs/core/src/goals.rs)
- [Continuation prompt template](/Users/nes0903/Documents/reference/codex/codex-rs/core/templates/goals/continuation.md)
- [Budget limit prompt template](/Users/nes0903/Documents/reference/codex/codex-rs/core/templates/goals/budget_limit.md)
- [Goal tool definitions](/Users/nes0903/Documents/reference/codex/codex-rs/core/src/tools/handlers/goal_spec.rs)
- [Goal tool handlers](/Users/nes0903/Documents/reference/codex/codex-rs/core/src/tools/handlers/goal.rs)
